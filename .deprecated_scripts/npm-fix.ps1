...
Write-Host "  首先只安裝 Next.js 和 React..." -ForegroundColor $YELLOW
npm install next@13.5.4 react@18.2.0 react-dom@18.2.0 --no-fund --no-audit --legacy-peer-deps

# Step 3: 安裝 UI 和工具依賴項
Write-Host "Step 3: 安裝 UI 和工具依賴項..." -ForegroundColor $CYAN
$uiDeps = @(
...
npm install $uiDeps --no-fund --no-audit --legacy-peer-deps

Write-Host "  安裝工具套件..." -ForegroundColor $YELLOW
npm install $toolDeps --no-fund --no-audit --legacy-peer-deps

# Step 4: 安裝開發套件
Write-Host "Step 4: 安裝開發依賴項..." -ForegroundColor $CYAN
$devDeps = @(
    "@types/node@^20",
    "@types/react@^18",
    "@types/react-dom@^18",
...
npm install $devDeps --save-dev --no-fund --no-audit --legacy-peer-deps
